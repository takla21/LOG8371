 ...
 * @version $Revision$
 */
public class FPGrowthCustom extends AbstractAssociator implements
  ...
  /**
   * Construct a new FPGrowthCustom object.
   */
  public FPGrowthCustom() {
    resetOptions();
  }
  ...
  public String transactionsMustContainTipText() {
    return "Limit input to FPGrowthCustom to those transactions (instances)"
  ...
  private Instances parseTransactionsMustContain(Instances data) {
    ...
    for (String element : split) {
      ...
      if (att == null) {
        System.err.println("[FPGrowthCustom] : WARNING - can't find attribute "
          + attName + " in the data.");
   ...
   *          data that FPGrowthCustom requires will be done off of disk (i.e. only
  ...
  public String toString() {
    ...
    if (m_rules == null) {
      return "FPGrowthCustom hasn't been trained yet!";
    }
    ...
    if (m_rules.size() == 0) {
      ...
      result.append("FPGrowthCustom found " + m_rules.size() + " rules");
      if (!m_findAllRulesForSupportLevel) {
  ...
  public static void main(String[] args) {
    try {
      ...
      if (Utils.getFlag('h', argsCopy) || Utils.getFlag("help", argsCopy)) {
        runAssociator(new FPGrowthCustom(), args);
        System.out
      ...
      if (!Utils.getFlag("disk", args)) {
        ...
        FPGrowthCustom FPGrowthCustom = new FPGrowthCustom();
        FPGrowthCustom.setOptions(args);
        Utils.checkForRemainingOptions(args);
        FPGrowthCustom.buildAssociations(loader);
        System.out.print(FPGrowthCustom.toString());
      }
    } catch (Exception ex) {
      ex.printStackTrace();
    }
